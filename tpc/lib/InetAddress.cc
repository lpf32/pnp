...
std::string InetAddress::toIpPort() const
{
    ...
    snprintf(buf+end, sizeof(buf)-end, ":%u", portHostEndian());
    return buf;
}

#ifdef __MACH__

bool InetAddress::resolve(StringArg hostname, InetAddress *addr)
{
    struct hostent *hent;
    hent = gethostbyname(hostname.c_str());
    if (strlen(hent->h_addr) != 0 && hent->h_addrtype == AF_INET)
    {
        addr->saddr_.sin_addr = *reinterpret_cast<struct in_addr*>(hent->h_addr);
        return true;
    }
